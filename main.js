...
global.db = new Low(
  /https?:\/\//.test(opts['db'] || '') ?
    new cloudDBAdapter(opts['db']) : /mongodb/.test(opts['db']) ?
      new mongoDB(opts['db']) :
      new JSONFile(`src/dbnya/database.json`)
)
...
// }
const authFile = `${sessionName}.json`
...
const connectionOptions = {
  ...
  version: [2, 2204, 13],
  browser: ['Nazuna -MD', 'Safari', '1.0.0']
}
...
await conn.hehe("6289530291942@s.whatsapp.net", global.fkontak).catch(err => { return !0 })
}

if (connection == 'close')
console.log(chalk.red('⏹️Koneksi berhenti dan mencoba menghubungkan kembali...'))
