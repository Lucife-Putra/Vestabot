...
global.waifu = pickRandom(global.waifuu)
//if (!m) return
...
if (user) {
	...
	if (!user.registered) {
	if (!('name' in user))
	user.name = this.getName(m.sender)
	if (!('email' in user))
	user.email = ''
	if (!('label' in user))
	user.label = ''
...
if (chat) {
	if (!('name' in chat))
	chat.name = this.getName(m.chat)
	if (!('closeGroup' in chat))
	chat.closeGroup = false
	if (!isNumber(chat.add))
	chat.add = 0
	if (!('isBanned' in chat))
	chat.isBanned = false
	if (!('welcome' in chat))
	...
	if (!('desc' in chat))
	chat.desc = true
	if (!('descUpdate' in chat))
	chat.descUpdate = true
	if (!('stiker' in chat))
	chat.stiker = false
	if (!('delete' in chat))
	chat.delete = false
...
const isROwner = [global.conn.user.jid, ...global.numb].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
...
if (e.name)
for (let jid of global.numb.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').filter(v => v != this.user.jid)) {
...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || `Hi, ${await this.getName(user)}!\nSelamat Datang Di ${await this.getName(id)}`).replace('@subject', groupMetadata.subject).replace('@desc', groupMetadata.desc.toString()) :
...
//console.log('=============\n\ngroupsUpdate \n\n============\n\n' + await groupUpdate)
if (!text) continue
await this.sendButtonDoc(id, text, wm, 'Matikan Fitur', `.off detect`, fkontak, { contextInfo: global.adReply.contextInfo, mentions: await this.parseMention(text) })
}
},
...
owner: `[ *AKSES DI TOLAK* ]\nPerintah Ini Hanya Dapat Di Gunakan Oleh Ownerrr!?!?`,
...
if (msgg) return conn.sendButtonDoc(m.chat, `    ▣═━–〈 𝐕𝐄𝐑𝐈𝐅𝐘 〉–━═▣`, msgg, '𝑽𝒆𝒓𝒊𝒇𝒚', '.verify', m, { contextInfo: { externalAdReply: { showAdAttribution: true,
    mediaUrl: ig,
    mediaType: "VIDEO",
    description: urlnya,
    title: `Hai Kak ${users}, ${ucapWaktu}`,
    body: wm,
    thumbnail: thumb,
    sourceUrl: urlnya
}}
