...
let handler = async (m, { conn, args, usedPrefix, command, participants }) => {
  ...
  try {
    ...
    if (/webp|image|video/g.test(mime)) {
      ...
      let out
      try {
      	conn.fakeReply(m.chat, `Wait Process...` , '0@s.whatsapp.net', '「 𝐒𝐓𝐈𝐂𝐊𝐄𝐑 」𝐒𝐚𝐛𝐚𝐫 𝐁𝐫𝐮𝐡🗿', 'status@broadcast')
    ...
    if (stiker) conn.sendFile(m.chat, stiker, 'sticker.webp', '', m, { mentions: participants.map(a => a.id) })
