...
handler.before = async function (m) {
  ...
  if (m.text.toLowerCase() == json.jawaban.toLowerCase()) {
    ...
    this.sendButton(m.chat, benar + ` +${this.caklontong[id][2]} Money\n${json.deskripsi}`, wm, 'Lagi', '.caklontong', m)
    clearTimeout(this.caklontong[id][3])
    delete this.caklontong[id]
  } else if (similarity(m.text.toLowerCase(), json.jawaban.toLowerCase().trim()) >= threshold) this.sendButton(m.chat, 'Sedikit Lagi', wm, 'Bantuan', '.calo', m)
  else this.sendButton(m.chat, 'Jawaban Salah', wm, 'Bantuan', '.calo', m)
  }
  return !0
}
handler.exp = 0
