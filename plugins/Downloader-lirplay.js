...
let handler = async (m, { conn, command, usedPrefix, text, isPrems, isOwner }) => {
  ...
  let { dl_link, thumbs, title, filesize, filesizeF } = yt
  let isLimit = (isPrems || isOwner ? 99 : limit) * 1024 < filesize
  conn.sendFile(m.chat, thumbs, 'thumbnail.jpg', `
*Title:* ${title}
*Filesize:* ${filesizeF}
*Source:* ${vid.url}
...
`.trim(), m)
let _thumbs = {}
try { if (isVideo) _thumbs = { thumbnail: await (await fetch(thumbs)).buffer() } }
catch (e) { }
